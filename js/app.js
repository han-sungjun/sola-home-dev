...
function preservePageScrollForDialogOpen(modal, savedY){
 const y = Math.max(0, Number(savedY) || 0);
 window.__upickPendingModalScrollY = y;
 try{
   if(typeof window.__upickLockModalBackgroundAt === 'function') window.__upickLockModalBackgroundAt(y);
   else if(typeof window.__upickSyncModalScrollLock === 'function') window.__upickSyncModalScrollLock();
 }catch(_){}
 requestAnimationFrame(() => {
...
function closeDetailDialogPreservingPage(modal){
 ...
 window.__upickDetailCloseInProgress = true;
 try{ if('scrollRestoration' in history) history.scrollRestoration = 'manual'; }catch(_){}
 try{
 ...
 // v85: body fixed/top 스타일은 dialog.close() 직후까지 유지합니다.
 // close 이벤트 안에서 복구 보정이 먼저 실행되면 body.top이 지워져 최상단으로 튀므로,
 // 닫힘 이벤트의 holdStablePageScrollY는 __upickDetailCloseInProgress 동안 우회합니다.
 try{
   if(modal){
 ...
 try{
   ...
   else restoreStablePageScrollY(y);
 }catch(_){
   restoreStablePageScrollY(y);
 }
 resetDetailDialogScroll(modal);
 requestAnimationFrame(() => restoreStablePageScrollY(y));
 setTimeout(() => restoreStablePageScrollY(y), 0);
 setTimeout(() => {
   window.__upickDetailCloseInProgress = false;
   holdStablePageScrollY(y, 450);
 }, 60);
 setTimeout(() => {
   window.__upickClosingDetailScrollY = undefined;
 }, 700);
}
