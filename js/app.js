...
window.addEventListener('keydown', (event) => {
 ...
 // 입장하기 직후 또는 화면 전환 직후 한 번 더 직접 동기화합니다.
 try{
 const roleMeta = getAccountRoleMeta(state.currentUserProfile || {});
 const selectors = [
...
async function askAiAssistant(rawQuestion=''){
 ...
 await openModalAlert('나가기 처리 중 오류가 발생했습니다.');
 }
 }
...
document.addEventListener('keydown', (event) => {
 ...
 qs('#logoutBtn').onclick=async()=>{const confirmed=await openModalConfirm('나가시겠습니까?', qs('#logoutBtn'), '나가기', '나가기', '취소'); if(!confirmed) return; try{await logoutServerSession(auth, API_URL[ENV]); await signOut(auth);redirectToLogin();}catch(error){await openModalAlert('나가기 처리 중 오류가 발생했습니다.', qs('#logoutBtn'));}};
