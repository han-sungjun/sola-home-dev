...
function preservePageScrollForDialogOpen(modal, savedY){
 ...
 try{
   if(typeof window.__upickLockModalScrollAt === 'function') window.__upickLockModalScrollAt(y);
...
function closeDetailDialogPreservingPage(modal){
 ...
 setTimeout(() => restoreStablePageScrollY(y), 80);
}
...
${item.content || ''}`);
 ...
 try{ if(typeof window.__upickLockModalScrollAt === 'function') window.__upickLockModalScrollAt(pageScrollY); }catch(_){}
