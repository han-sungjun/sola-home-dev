...
(function(){
  var scrollY = 0;
  var locked = false;
  var lockClasses = ['upick-modal-hard-lock'];
  var allowedSelector = [
  ...
  function removeLockClasses(){
    ['upick-modal-hard-lock','upick-v9-modal-lock','upick-modal-lock','upick-modal-open'].forEach(function(cls){
  ...
  function lock(presetY){
    ...
    }else if(locked) nextY = scrollY || getBodyTopY() || 0;
    else nextY = getCurrentY();
  ...
  function unlock(forcedY){
    ...
    // 중요: body top 값을 읽은 뒤, lock class를 제거하고 같은 tick 안에서 즉시 scroll을 복구합니다.
    // class 제거 전 top:0이 되면 화면이 최상단에 고정되므로 top을 0으로 재설정하지 않습니다.
    removeLockClasses();
    ...
    setTimeout(function(){ restoreWindowY(restoreY); }, 80);
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if(window.MutationObserver){
      ...
      // style/class 감시는 제외합니다. lock/unlock 자체가 style/class를 바꾸기 때문에 루프가 생깁니다.
      mo.observe(document.body, {subtree:true, attributes:true, attributeFilter:['open','aria-hidden']});
    }
  });
