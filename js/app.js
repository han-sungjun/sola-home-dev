...
function formatTravelDuration(minutes){
 ...
 let benefitDistanceAutoRequested = false;
 function refreshBenefitDistancesInBackground(){
 if(benefitDistanceAutoRequested) return;
 if(!navigator.geolocation) return;
 if(!state.benefits || !state.benefits.length) return;
 benefitDistanceAutoRequested = true;
 ...
 .catch((error) => {
 console.warn('초기 거리 표시 준비 실패', error);
 if(state.distanceStatus !== 'error') state.distanceStatus = 'idle';
 updateDistanceFilterHelp();
 });
 }
...
stationAccessText:item.stationAccessText||item.transitText||item.stationGuide||item.nearStationText||'',
 ...
 state.loading = false;
 renderAll();
 refreshBenefitDistancesInBackground();
 handleCleanDeepLink();
...
${item.content || ''}`);
 ...
 ? '아직 저장한 혜택이 없습니다.<br><small>관심 있는 매장의 별표를 눌러 저장해 보세요.</small>'
 : (state.keyword || state.category !== '전체' || state.filter !== 'all' || state.distanceRadius !== 'all'
 ? '검색 조건에 맞는 혜택을 찾지 못했습니다.<br><small>검색어나 필터를 조금 바꿔보세요.</small>'
 : '현재 표시할 혜택이 없습니다.<br><small>새로운 혜택이 등록되면 이곳에 표시됩니다.</small>');
