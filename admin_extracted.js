...
const setTextAll = (selector, text) => qsa(selector).forEach(el => el.textContent = text);
    ...
    function addSupportProgramRow(row={}){
      ...
      rows.push({name:row.name||'',startedAt:row.startedAt||'',endedAt:row.endedAt||''});
      renderSupportProgramRows(rows);
      const last=qs('#supportProgramRows [data-support-program-row]:last-child .support-program-name-input');
      if(last) last.focus();
    }
